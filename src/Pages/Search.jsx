...
function Search() {
  ...
  const searchMovies = async (searchTerm) => {
    ...
    const tagsQuery = query(moviesRef, where("tags", "array-contains", lowerSearchTerm));

    const genreQuery = query(moviesRef, where("genre", "array-contains", lowerSearchTerm));
    ...
    try {
      const [titleSnap, tagsSnap, genreSnap, categorySnap] = await Promise.all([
        getDocs(titleQuery),
        getDocs(tagsQuery),
        getDocs(genreQuery),
      ...
      const results = [
        ...titleSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })),
        ...tagsSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })),
        ...genreSnap.docs.map(doc => ({ id: doc.id, ...doc.data() })),
        ...categorySnap.docs.map(doc => ({ id: doc.id, ...doc.data() })),
      ];

      const uniqueResults = Array.from(new Map(results.map(movie => [movie.title, movie])).values());
